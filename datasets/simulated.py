...
# Dataset iterator
def dataset_generator(data, rng=None, batch_size=200):
    if rng is None:
    ...
    if data == "swissroll":
        ...
        return dataset_generator("8gaussians", rng, batch_size)
...
class Dataset(BaseDataset):
    ...
    def get_data(self):
        ...
        X = dataset_generator(data=self.name, batch_size=self.n_samples)
        X = torch.FloatTensor(X)
        # Split train/test
        X_train, X_test = torch.utils.data.random_split(
            X, [int(len(X) * (1 - self.test_size)), int(len(X) * self.test_size)]
        )
